...
namespace WinForms.JumpLists.JumpLists;
...
public sealed class JumpItemsRejectedEventArgs : EventArgs
{
    ...
    public JumpItemsRejectedEventArgs(IList<JumpItem> rejectedItems, IList<JumpItemRejectionReason> reasons)
    {
        ...
        {
            throw new ArgumentException(ApplicationStrings.JumpItemsRejectedEventArgs_CountMismatch);
        }
...
public sealed class JumpList : ISupportInitialize
{
    ...
    private class ShellObjectPair
    {
        ...
        public static void ReleaseShellObjects(List<ShellObjectPair> list)
        {
            if (list != null)
            {
                foreach (ShellObjectPair shellMap in list)
                {
    ...
    /// </summary>
    // Added by Bradford Bruce, to support Winforms
    // use System.Diagnostics.Process.GetCurrentProcess().Handle.ToString(); as the application identifier
    public static void SetJumpList(string application, JumpList value)
    {
    ...
    /// </summary>
    public static JumpList GetJumpList(string application)
    {
        Verify.IsNotNull(application, "application");
    ...
    private static readonly Dictionary<string, JumpList> s_applicationMap = new();
    ...
    public void BeginInit()
    {
        if (!IsUnmodified)
        {
            throw new InvalidOperationException(ApplicationStrings.JumpList_CantNestBeginInitCalls);
        }
    ...
    public void EndInit()
    {
        ...
        {
            throw new NotSupportedException(ApplicationStrings.JumpList_CantCallUnbalancedEndInit);
        }
    ...
    public void Apply()
    {
        if (_initializing == true)
        {
            throw new InvalidOperationException(ApplicationStrings.JumpList_CantApplyUntilEndInit);
        }
    ...
    public static string CurrentApplication { get { return System.Diagnostics.Process.GetCurrentProcess().Id.ToString(); } }
    ...
    private void ApplyFromApplication()
    {
        ...
        if (_application == CurrentApplication && _initializing == false)
        {
    ...
    private void ApplyList()
    {
        ...
        List<RejectedJumpItemPair> rejectedList;

        // Declare these outside the try block so we can cleanup native resources in the _ShellObjectPairs.
        List<List<ShellObjectPair>> categories = null;
        List<ShellObjectPair> removedList = null;
        ...
        try
        {
            ...
            rejectedList = new List<RejectedJumpItemPair>(JumpItems.Count);
            ...
            categories = new List<List<ShellObjectPair>>() { new List<ShellObjectPair>() };
            ...
            foreach (var jumpItem in JumpItems)
            {
                if (jumpItem == null)
                {
                    ...
                    rejectedList.Add(new RejectedJumpItemPair { JumpItem = jumpItem, Reason = JumpItemRejectionReason.InvalidItem });
                    continue;
                }
                ...
                try
                {
                    ...
                    if (shellObject == null)
                    {
                        rejectedList.Add(new RejectedJumpItemPair { Reason = JumpItemRejectionReason.InvalidItem, JumpItem = jumpItem });
                        continue;
                    }
                    ...
                    {
                        rejectedList.Add(new RejectedJumpItemPair { Reason = JumpItemRejectionReason.RemovedByUser, JumpItem = jumpItem });
                        continue;
                    }

                    var shellMap = new ShellObjectPair { JumpItem = jumpItem, ShellObject = shellObject };
                    ...
                    else
                    {
                        ...
                        if (!categoryExists)
                        {
                            categories.Add(new List<ShellObjectPair>() { shellMap });
                        }
                    }
            ...
            foreach (List<ShellObjectPair> categoryList in categories)
            {
        ...
        finally
        {
            ...
            if (categories != null)
            {
                foreach (List<ShellObjectPair> list in categories)
                {
                    ShellObjectPair.ReleaseShellObjects(list);
                }
            }
            ...
            ShellObjectPair.ReleaseShellObjects(removedList);
        }
        ...
        if (rejectedList.Count > 0 && rejectedHandler != null)
        {
            ...
            foreach (RejectedJumpItemPair rejectionPair in rejectedList)
            {
        ...
        if (removedList.Count > 0 && removedHandler != null)
        {
            ...
            foreach (ShellObjectPair shellMap in removedList)
            {
    ...
    private static bool ListContainsShellObject(List<ShellObjectPair> removedList, object shellObject)
    {
    ...
    private static List<ShellObjectPair> GenerateJumpItems(IObjectArray shellObjects)
    {
        Debug.Assert(shellObjects != null);

        var retList = new List<ShellObjectPair>();
        ...
        for (uint i = 0; i < count; ++i)
        {
            ...
            retList.Add(new ShellObjectPair { ShellObject = unk, JumpItem = item });
        }

        return retList;
    }

    private static void AddCategory(ICustomDestinationList cdl, string category, List<ShellObjectPair> jumpItems, List<JumpItem> successList, List<RejectedJumpItemPair> rejectionList)
    {
        AddCategory(cdl, category, jumpItems, successList, rejectionList, true);
    }

    private static void AddCategory(ICustomDestinationList cdl, string category, List<ShellObjectPair> jumpItems, List<JumpItem> successList, List<RejectedJumpItemPair> rejectionList, bool isHeterogenous)
    {
        ...
        else
        {
            ...
            if (isHeterogenous && hr == HRESULT.DESTS_E_NO_MATCHING_ASSOC_HANDLER)
            {
                ...
                var linksOnlyList = new List<ShellObjectPair>();
                foreach (var itemMap in jumpItems)
                {
                    if (itemMap.JumpItem is JumpPath)
                    {
                        rejectionList.Add(new RejectedJumpItemPair { JumpItem = itemMap.JumpItem, Reason = JumpItemRejectionReason.NoRegisteredHandler });
                    }
                    else
                    {
            ...
            else
            {
                ...
                {
                    rejectionList.Add(new RejectedJumpItemPair { JumpItem = item.JumpItem, Reason = JumpItemRejectionReason.InvalidItem });
                }
            }
        }
    }
