...
namespace LanguagePuller
{
    public partial class Form1 : Form
    {
        ...
        protected override void OnShown(EventArgs e)
        {
            ...
            int num = ToLines(languages);

            ComboBoxItemList items = ToComboBoxItems(lines);
            ...
            AutoCompleteStringCollection sc = new();
            sc.AddRange(items.Values.ToArray());
        ...
        private int ToLines(string text)
        {
            lines.Clear();
        ...
        ComboBoxItemList ToComboBoxItems(List<string> lines)
        {
            ComboBoxItemList items = new();

            foreach (string line in lines)
            {
                string[] parts = line.Split('|');

                items.Add(new ComboBoxItem { Text = parts[1].Trim(), Value = fixAlias(parts[2]).Trim() });
            }

            // remove the heading items
            items.RemoveAt(0);
            items.RemoveAt(0);
        ...
        string languages = @"| Language                | Aliases                | Package |
| :-----------------------| :--------------------- | :------ |
| 1C                      | 1c                     |         |
...
| Zig                     | zig                    |[highlightjs-zig](https://github.com/fwx5618177/highlightjs-zig) |";
        ...
        {
            string selText = textBox2.SelectedText;


            string test = GetLineBefore(textBox2);
        }
