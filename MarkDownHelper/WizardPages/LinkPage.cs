...
{
    public partial class LinkPage : WizardPage
    {
        public LinkPage()
        {
            InitializeComponent();
        ...
        public override void BeforeNextPage(WizardPageEventArgs args)
        {
            ...
            if (!string.IsNullOrEmpty(link))
            {
                ...
                if (!string.IsNullOrEmpty(tooltip))
                {
                    //linkText = string.Format("{0} \"{1}\"", link, tooltip);
                    linkText = $"{link} \"{tooltip}\"";
                }

                //newText = string.Format("[{0}]({1})", display, linkText);
                newText = $"[{display}]({linkText})";
            }
